...
export const GET = withAuth(async (_req, { params, userId }) => {
  ...
  const [spreadRows, legs, funding, notes] = await Promise.all([
    ...
    `,
    sql`SELECT * FROM public.notes WHERE activity_id = ${id}::uuid AND user_id = ${userId}::uuid LIMIT 1`,
  ]);
...
export const PATCH = withAuth(async (req, { params, userId }) => {
  ...
  // After the activity supertype migration, all three editable fields live on
  // the `activity` table (the supertype), not on `activity_spread`.
  const rows = await sql`
    ...
    WHERE id = ${id}::uuid AND user_id = ${userId}::uuid AND type = 'spread'
    RETURNING *
  `;
  if (!rows[0]) return errors.notFound();

  const [pnlRow] = await sql`
    SELECT * FROM public.spread_pnl WHERE spread_id = ${id}::uuid AND user_id = ${userId}::uuid LIMIT 1
  `;
  return ok(pnlRow ?? rows[0]);
});
