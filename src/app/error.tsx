...
/**
 * Root /error boundary — catches any uncaught render error from server
 * components or actions inside the segment. Must be a client component
 * (Next.js requirement). The `reset` callback re-runs the failing
 * server tree once the user dismisses the error.
 */
...
export default function GlobalError({ error, reset }: Props) {
  ...
  React.useEffect(() => {
    // Console for the user; the digest is what they'd quote to support.
    console.error("[csj] page boundary error", error);
  }, [error]);
